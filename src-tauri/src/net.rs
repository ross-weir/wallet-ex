//! Port picking code taken from: https://github.com/Dentosal/portpicker-rs
...
fn try_bind_udp<A: ToSocketAddrs>(addr: A) -> Option<Port> {
  Some(UdpSocket::bind(addr).ok()?.local_addr().ok()?.port())
}

// Try to bind to a socket using TCP
fn try_bind_tcp<A: ToSocketAddrs>(addr: A) -> Option<Port> {
...
pub fn is_free_udp(port: Port) -> bool {
  ...
  try_bind_udp(ipv6).is_some() && try_bind_udp(ipv4).is_some()
}
...
fn ask_free_tcp_port() -> Option<Port> {
  ...
  try_bind_tcp(ipv6).or_else(|| try_bind_tcp(ipv4))
}
...
pub fn pick_unused_port() -> Option<Port> {
  ...
  for _ in 0..10 {
    let port = rng.gen_range(25000..35000);
    if is_free(port) {
      return Some(port);
    }
  }
